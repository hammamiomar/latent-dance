...
Handles: UpdateBlockConfig (the unified v2 block-centric message).
"""
...
from app.schemas import UpdateBlockConfig
...
def handle_stem_message(
    strategy: "StrategyProtocol",
    message: BaseModel,
) -> Optional[dict]:
    """Handle block configuration messages."""
    if isinstance(message, UpdateBlockConfig):
        return _handle_update_block_config(strategy, message)
    return None


def _handle_update_block_config(
    strategy: "StrategyProtocol",
    message: UpdateBlockConfig,
) -> None:
    """Update block-centric configuration."""
    config = strategy.slot_configs.get(message.block)
    if config is None:
        config = BlockLinkConfig(
            block=message.block,
            feature_id=message.feature_id or 0,
        )
        strategy.slot_configs[message.block] = config
    ...
    if message.spatial_mask is not None:
        ...
        if hasattr(strategy, '_spatial') and strategy._spatial is not None:
            strategy._spatial.set_block_mask(message.block, message.spatial_mask)
    ...
    if strategy._physics is not None and strategy.stem_features:
        if config.link_target != link_target_before or config.physics_preset != physics_before:
            try:
                ...
                logger.exception("Failed to reinitialize physics after block update")

    logger.info(
        f"UpdateBlockConfig: block={message.block}, link_target={config.link_target}, "
...
def is_stem_message(message: BaseModel) -> bool:
    """Check if message is a block configuration message."""
    return isinstance(message, UpdateBlockConfig)
