...
mkdir -p /workspace/.cache/hambajuba2ba/songs
mkdir -p "${TORCHINDUCTOR_CACHE_DIR:-/root/.cache/torchinductor}"

case "${MODE:-api}" in
    api)
        echo "Starting API server on :8000"
        cd /app
        exec uv run --frozen --no-dev --extra audio-gpu uvicorn app.main:app --host 0.0.0.0 --port 8000
        ;;
    shell)
